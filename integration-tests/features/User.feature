Feature: Features related to User Management
  ...
  @updateUserById
  Scenario: PUT to /organisations/{organisationId}/users/{userId} with pre-conditioned knownUserId and valid body returns 204
  ...
  Scenario: PUT to /organisations/{organisationId}/users/{userId} with pre-conditioned knownOrgId and invalid body returns 400
    Given I generate a json payload called 'badUserModificationRequest'
    When I make a PUT to the function at '/organisations/knownOrgId/users/knownUserId'
    Then I should get a status code 400

  @updateUserById @errorHandling
  Scenario: PUT to /organisations/{organisationId}/users/{userId} with pre-conditioned unknownOrganisationId returns 404
    Given I generate a json payload called 'userModificationRequest'
